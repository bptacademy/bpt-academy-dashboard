...
export default function TournamentManageScreen({ navigation }: any) {
  ...
  const handleCreate = async () => {
    ...
    setForm({ title: '', description: '', start_date: '', end_date: '', location: '', entry_fee_gbp: '', max_participants: '', registration_deadline: '', registration_opens_at: '', eligible_divisions: ['semi_pro', 'pro'] });
    fetchTournaments();
  };
          ...
          {tournaments.map(t => {
            ...
            return (
              <View key={t.id} style={styles.card}>
                ...
                {regOpensAt && (
                  <View style={styles.regOpenRow}>
                    <Text style={[styles.cardMeta, { color: regOpen ? '#4ADE80' : '#F59E0B' }]}>
                      🗓 Registration opens: {formatDate(regOpensAt)}
                      {regOpen ? ' ✅ Open' : ' ⏳ Not yet'}
                    </Text>
                  </View>
                )}
        ...
        <ScrollView style={styles.modal}>
          ...
          <View style={styles.form}>
            {[
              ...
              { label: 'Registration Opens On', key: 'registration_opens_at', placeholder: 'YYYY-MM-DD (players can register from this date)' },
...
const styles = StyleSheet.create({
  ...
  regOpenRow: { marginTop: 2, marginBottom: 2 },
  ...
  input: {
    ...
    backgroundColor: 'rgba(17,30,51,0.85)', marginBottom: 4,
  },
  ...
  divChip: {
    ...
    paddingHorizontal: 14, paddingVertical: 8, backgroundColor: 'rgba(255,255,255,0.05)',
  },
