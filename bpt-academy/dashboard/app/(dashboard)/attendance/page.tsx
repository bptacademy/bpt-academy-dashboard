  ...
  async function handleSessionChange(sessionId: string) {
    ...
    if (enrollments) {
      for (const e of enrollments) {
        const p = e.profiles as { id?: string; full_name?: string; email?: string } | null
        if (p?.id) {
          studentList.push({
            id: p.id,
    ...
    // Default absent for students without record
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Mark session attendance for enrolled students
        </p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="py-8 text-center text-gray-400">
              No enrolled students for this program
            </div>
          ) : (
        ...
        <div className="grid grid-cols-3 gap-4">
          {(['present', 'late', 'absent'] as AttendanceStatus[]).map((status) => {
            ...
            return (
              <div
                key={status}
                className={`rounded-xl border p-4 text-center ${config.color}`}
              >
