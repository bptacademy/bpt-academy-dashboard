/**
 * OTCConnectScreen — connects an existing volpair user to On The Court
 * Used from Profile → Sync History → Connect (for users who already have a profile)
 * No onboarding questions — profile already complete.
 * When real OTC API is available, this screen gains a credential form / WebView.
 */
import React, { useState } from 'react';
import {
  View, Text, TouchableOpacity, StyleSheet,
  StatusBar, ScrollView, ActivityIndicator, Alert,
  ImageBackground,
} from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { theme, fonts } from '../../lib/theme';
import { useAuth } from '../../context/AuthContext';
import { supabase } from '../../lib/supabase';
const _BG = require('../../../assets/volpair-bg-v2.png');

const SUPABASE_URL = 'https://qmdewocktouqoibbqurh.supabase.co';

export default function OTCConnectScreen({ navigation }: any) {
  const insets = useSafeAreaInsets();
  const { user } = useAuth();
  const [loading, setLoading] = useState(false);

  const handleConnect = async () => {
    if (!user?.id) return;
    setLoading(true);
    try {
      const session = await supabase.auth.getSession();
      const token = session.data.session?.access_token;
      if (!token) throw new Error('Not authenticated');

      // Register OTC connection (manual mode — no API auth yet)
      const res = await fetch(`${SUPABASE_URL}/functions/v1/platform-auth`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
        body: JSON.stringify({
          platform: 'on_the_court',
          otcUserId: user.id,
        }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? 'Could not connect OTC account');

      // Trigger score recompute in background
      fetch(`${SUPABASE_URL}/functions/v1/compute-scores`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': 'sb_publishable_KwkQawb1Kv2jOk1Wud0xUg_mPQxPqmL',
        },
        body: JSON.stringify({ user_id: user.id }),
      }).catch(() => {});

      Alert.alert(
        '✅ Connected!',
        'On The Court has been connected. Your volpair Score will update shortly.',
        [{ text: 'Done', onPress: () => navigation.goBack() }]
      );
    } catch (e: any) {
      Alert.alert('Error', e?.message ?? 'Could not connect. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  return (
    <ImageBackground source={_BG} style={{ flex: 1 }} resizeMode="cover">
    <View style={{flex:1, backgroundColor:'transparent'}}>
      <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      
      <ScrollView showsVerticalScrollIndicator={false}>

        <TouchableOpacity style={styles.backBtn} onPress={() => navigation.goBack()}>
          <Text style={styles.backText}>← Back</Text>
        </TouchableOpacity>

        <View style={styles.header}>
          <View style={styles.platformBadge}>
            <Text style={styles.platformBadgeText}>🎯 On The Court</Text>
          </View>
          <Text style={styles.title}>Connect On The Court</Text>
          <Text style={styles.subtitle}>
            Link your On The Court account to improve your volpair Score with real booking history — courts you play at, partners you book with, and your preferred schedule.
          </Text>
        </View>

        <View style={styles.infoBox}>
          <Text style={styles.infoTitle}>🏗️ What gets imported</Text>
          <View style={styles.infoList}>
            <Text style={styles.infoItem}>✅ Courts and clubs you play at</Text>
            <Text style={styles.infoItem}>✅ Booking partners (players you've played with)</Text>
            <Text style={styles.infoItem}>✅ Preferred days and times</Text>
            <Text style={styles.infoItem}>🔜 Full match history (coming with API integration)</Text>
          </View>
        </View>

        <View style={styles.noteBox}>
          <Text style={styles.noteText}>
            Full On The Court API integration is in development. Connecting now registers your account and will automatically sync your full match history when the integration launches.
          </Text>
        </View>

        <TouchableOpacity
          style={[styles.connectBtn, loading && styles.connectBtnDisabled]}
          onPress={handleConnect}
          disabled={loading}
          activeOpacity={0.85}
        >
          {loading
            ? <ActivityIndicator color={theme.bg} />
            : <Text style={styles.connectBtnText}>Connect On The Court</Text>
          }
        </TouchableOpacity>

      </ScrollView>
    </View>
    </View>
    </ImageBackground>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: 'transparent', paddingHorizontal: 24 },
  backBtn:            { paddingVertical: 16 },
  backText:           { color: theme.textSecondary, fontSize: 17.1, fontFamily: fonts.bodyLight },
  header:             { marginBottom: 24 },
  platformBadge:      {
    alignSelf: 'flex-start', backgroundColor: 'rgba(0,212,200,0.1)',
    borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 16,
  },
  platformBadgeText:  { color: theme.primary, fontFamily: fonts.bodyBold, fontSize: 13.9 },
  title:              { fontSize: 28, fontFamily: fonts.headlineBold, color: theme.textPrimary, marginBottom: 10 },
  subtitle:           { fontSize: 15, color: theme.textMuted, lineHeight: 22, fontFamily: fonts.bodyLight },
  infoBox:            {
    backgroundColor: theme.bgCard, borderRadius: 16, padding: 20,
    borderWidth: 1.5, borderColor: theme.border, marginBottom: 16,
  },
  infoTitle:          { fontSize: 16.1, fontFamily: fonts.bodyBold, color: theme.textPrimary, marginBottom: 12 },
  infoList:           { gap: 8 },
  infoItem:           { fontSize: 15, color: theme.textMuted, fontFamily: fonts.bodyLight, lineHeight: 20 },
  noteBox:            {
    backgroundColor: 'rgba(0,212,200,0.05)', borderRadius: 14, padding: 16,
    borderWidth: 1, borderColor: theme.primaryBorder, marginBottom: 28,
  },
  noteText:           { fontSize: 13.9, color: theme.textMuted, lineHeight: 20, fontFamily: fonts.bodyLight },
  connectBtn:         {
    backgroundColor: theme.primary, borderRadius: 16, paddingVertical: 20,
    alignItems: 'center',
    shadowColor: theme.primary, shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.3, shadowRadius: 10, elevation: 5,
  },
  connectBtnDisabled: { opacity: 0.5 },
  connectBtnText:     { color: '#0D1B2A', fontSize: 17, fontFamily: fonts.headlineBold },
});
